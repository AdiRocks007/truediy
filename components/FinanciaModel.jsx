...
const FinancialModel = () => {
  ...
  const { projectName, projectCost } = location.state || {};
  ...
  const [acquisitionPrice, setAcquisitionPrice] = useState(parseFloat({projectCost}));
  const [sliderMin, setSliderMin] = useState(0);
  const [sliderMax, setSliderMax] = useState(0);

  const [extraCharge, setextraCharge] = useState(150000);

  const [sellingPrice, setSellingPrice] = useState(2.25*parseFloat({projectCost}));
  ...
  useEffect(() => {
    ...
    const fetchedPrice = parseFloat(projectCost); // Replace with your actual fetch call
  ...
  useEffect(() => {
    // Fetch the acquisition price from the backend and set it
    const fetchedSPrice = 2.25*parseFloat(projectCost) // Replace with your actual fetch call
  ...
  const handlelogClick = () => {
    handleListItemClick(items.length);
    navigate('/');
  };
  ...
  const calculateAndFetch = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.post('https://true-int.vercel.app/api/uc_calculate_irr', formData, {
        headers: {
          'Content-Type': 'application/json',
          'Access-Control-Allow-Origin': 'https://true-int.vercel.app',
        },
      });
    ...
    <div style={{ display: 'flex' }}>
      ...
      >
        ...
        <List style={{ marginTop: 'auto' }}>
          ...
          >
            ...
            {open && <ListItemText primary={<Typography variant="body1">Go Back</Typography>} />}</div>
          </ListItemButton>
        </List>
      </Drawer>
      <Grid container>
      <Grid item xs={12}>
        ...
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Typography variant="h4" style={{ display: 'inline' }}>{projectName}</Typography>
